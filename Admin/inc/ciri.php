<?php
 if (empty($_SESSION['namauser']) AND empty($_SESSION['passuser'])){
  echo "<h1>ERROR!!! <br> <br> Maaf untuk mengakses halaman ini, Anda harus login dulu. <br> <br> Klik tombol di bawah ini</h1>
    ...
    default:
	echo "<h3>Halaman Ciri</h3>";
	echo "<p><input type='button' class='btn btn-primary' value='Tambah Ciri' onclick=window.location.href='?page=ciri&olah=tambahciri'></p>";
 	$query = "SELECT * FROM tb_ciri ORDER BY kd_ciri limit $posisi, $batas";
	  ...
	  echo "<div class='table-responsive'>
        ...
        $no++;
      	...
      	<?php
			...
			if ( $pg > 1 ) {
			$link = $pg - 1;
			$prev ="<a href='?page=ciri&pg=$link'>Sebelumnya </a>";
			} else {
			$prev = "Sebelumnya ";
			}
			...
			if ( $pg < $JmlHalaman ) {
			$link = $pg + 1;
			$next = " <a href='?page=ciri&pg=$link'>Selanjutnya</a>";
			} else {
			$next = " Selanjutnya";
			}
...
echo "total data Anda: $jml_data"; 
      ...
      echo "</tbody>
        ...
        case "tambahciri":
          echo "<h3>Tambah Ciri</h3>
          <form class='form-horizontal' method='POST' action='$proses?page=ciri&olah=input'>
		  ...
		  <div class='form-group'>
          <label for='inputText' class='col-xs-2'>Ciri</label>
		  <div class='col-xs-10'>     
		  <input type='text' class='form-control' id='inputText' name='ciri'>
		  </div>
		  </div>	
          ...
          break;
         case "editciri":
		      $query = "SELECT * FROM tb_ciri WHERE kd_ciri='$_GET[id]'";
		      $hasil = mysqli_query($koneksi, $query);
		      $data  = mysqli_fetch_array($hasil);

		  echo "<h3>Edit Ciri</h3>
          <form class='form-horizontal' method='POST' action='$proses?page=ciri&olah=edit'>		 
		  <div class='form-group'>
          <label for='inputText' class='col-xs-2'>Kode Ciri</label>
		  <div class='col-xs-10'>
		  <input type='text' class='form-control' id='inputText' name='kd_ciri' value='$data[kd_ciri]'>
		  </div>
		  </div>
